...
UserList(FundadorIndex).Stats.GLD = Abs(UserList(FundadorIndex).Stats.GLD) - 20000000
...
Public Function PuedeFundarUnClan(ByVal UserIndex As Integer, ByVal Alineacion As ALINEACION_GUILD, ByRef refError As String) As Boolean
    ...
    PuedeFundarUnClan = False
        ...
        End If
            ...
            If UserList(UserIndex).Stats.GLD < 20000000 Then
refError = "Para fundar clan necesitas 20.000.000 monedas de oro (20 millones)."
Exit Function
End If
    ...
    If UserList(UserIndex).Stats.ELV < 50 Or UserList(UserIndex).Stats.UserSkills(eSkill.Liderazgo) < 100 Then
        refError = "Para fundar un clan debes ser nivel máximo (50) y tener 100 en liderazgo."
        Exit Function
    End If
...
On Error GoTo errh
    Call SendData(SendTarget.ToAll, 0, 0, "||ZeusAO> Revisando elecciones pendientes." & FONTTYPE_SERVER)
    For i = 1 To CANTIDADDECLANES
        If Not Guilds(i) Is Nothing Then
            If Guilds(i).RevisarElecciones Then
                Call SendData(SendTarget.ToAll, 0, 0, "||ZeusAO> " & Guilds(i).GetLeader & " es el nuevo lider de " & Guilds(i).GuildName & "!" & FONTTYPE_SERVER)
            End If
        End If
proximo:
    Next i
    Call SendData(SendTarget.ToAll, 0, 0, "||ZeusAO> Elecciones revisadas correctamente." & FONTTYPE_SERVER)
Exit Sub
errh:
