...
If ObjIndex = 4295 Then
    If UserList(userindex).pos.Map = 117 Then
        Call SendData(SendTarget.toIndex, userindex, 0, "||¡No puedes usar el teletransportador, ya estás aqui! ~255~255~0~1~0~")
        Exit Sub
    End If
    Call WarpUserCharX(userindex, 117, 50, 72, True)
    Call SendData(SendTarget.toIndex, userindex, 0, "||¡Apurate guerrero tan solo tienes 30 minutos o hasta que la muerte lo dicte para entrenar en este mapa! ~255~255~0~1~0~")
    Call QuitarObjetos(4295, 1, userindex)
    
    Dim PosSpawn As WorldPos
    PosSpawn.Map = 117
    PosSpawn.x = 51
    PosSpawn.Y = 35
    If MazProIndex > 0 Then
        If Npclist(MazProIndex).Numero = 965 Then
            Call QuitarNPC(MazProIndex)
        End If
    End If
    MazProIndex = SpawnNpc(965, PosSpawn, False, False)
End If
'[/MaTeO 13]
'Actualiza
