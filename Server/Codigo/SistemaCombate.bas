...
Select Case UCase$(Clase)
    Case "GUERRERO"
        ModificadorEvasion = 0.91
    Case "CAZADOR"
        ModificadorEvasion = 0.82
    Case "PALADIN"
        ModificadorEvasion = 0.9
    Case "ASESINO"
        ModificadorEvasion = 0.98
    Case "BARDO"
        ModificadorEvasion = 0.96
    Case "MAGO"
        ModificadorEvasion = 0.56
    Case "CLERIGO"
        ModificadorEvasion = 0.86
    Case Else
...
Select Case UCase$(Clase)
    ...
    Case "ASESINO"
        ModificadorPoderAtaqueArmas = 0.94
    Case "CLERIGO"
        ModificadorPoderAtaqueArmas = 0.85
    Case "BARDO"
...
Select Case UCase$(Clase)
    ...
    Case "CAZADOR"
        ModificadorPoderAtaqueProyectiles = 0.84
    Case "PALADIN"
...
If UserList(userindex).Invent.EscudoEqpObjIndex > 0 Then
    If Not NpcImpacto Then
        If SkillDefensa + SkillTacticas > 0 Then  'Evitamos división por cero
            ...
            If Rechazo = True And UCase(UserList(userindex).Clase) <> "MAGO" Then
...
'[/MaTeO 3]
    ...
    If UsuarioImpacto = False Then
      ProbRechazo = Maximo(10, Minimo(90, 100 * (SkillDefensa / (SkillDefensa + SkillTacticas))))
      Rechazo = (RandomNumber(1, 100) <= ProbRechazo)
      If Rechazo = True And UCase(UserList(VictimaIndex).Clase) <> "MAGO" Then
      'Se rechazo el ataque con el escudo
