...
Dim DummyString     As String
    ...
    Else
        ...
        Exit Function
    End If
    Call QuitarObjetos(4296, 1, FundadorIndex)
    Call QuitarObjetos(979, 10, FundadorIndex)
UserList(FundadorIndex).Stats.GLD = Abs(UserList(FundadorIndex).Stats.GLD) - 5000000
...
Public Function PuedeFundarUnClan(ByVal userindex As Integer, ByVal Alineacion As ALINEACION_GUILD, ByRef refError As String) As Boolean
    ...
    PuedeFundarUnClan = False
        If TieneObjetos(979, 10, userindex) = False Then
            refError = "Necesitas 10 Piedras de Clan para poder crear un Clan." 'En esta línea pueden modifiar lo que está entre comillas por el mensaje que se retorna en caso de que el usuario no tenga el Item o la cantidad necesaria.
            Exit Function
        End If
         If TieneObjetos(4296, 1, userindex) = False Then
            refError = "Necesita 1 Cruz de Lider para poder crear un Clan." 'En esta línea pueden modifiar lo que está entre comillas por el mensaje que se retorna en caso de que el usuario no tenga el Item o la cantidad necesaria.
            Exit Function
        End If
            If UserList(userindex).Stats.GLD < 10000000 Then
refError = "Para fundar clan necesitas 10.000.000 monedas de oro (10 millones)."
Exit Function
End If
...
On Error GoTo errh
    Call SendData(SendTarget.toAll, 0, 0, "||RevivalAo> Revisando elecciones pendientes." & FONTTYPE_SERVER)
    For i = 1 To CANTIDADDECLANES
        If Not Guilds(i) Is Nothing Then
            If Guilds(i).RevisarElecciones Then
                Call SendData(SendTarget.toAll, 0, 0, "||RevivalAo> " & Guilds(i).GetLeader & " es el nuevo lider de " & Guilds(i).GuildName & "!" & FONTTYPE_SERVER)
            End If
        End If
proximo:
    Next i
    Call SendData(SendTarget.toAll, 0, 0, "||RevivalAo> Elecciones revisadas correctamente." & FONTTYPE_SERVER)
Exit Sub
errh:
