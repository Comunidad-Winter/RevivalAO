       ...
       If UserList(TempCharIndex).flags.AdminInvisible = 0 Or UserList(UserIndex).flags.Privilegios = PlayerType.Dios Then
            ...
            If UserList(TempCharIndex).DescRM = "" Then
                ...
                If UserList(TempCharIndex).Faccion.ArmadaReal = 1 Then
                    Stat = Stat & " (Seguidor Del REY) " & "[" & TituloReal(TempCharIndex) & "]"
                ElseIf UserList(TempCharIndex).Faccion.FuerzasCaos = 1 Then
                    Stat = Stat & " (Seguidor Del DEMONIO) " & "[" & TituloCaos(TempCharIndex) & "]"
                End If
                ...
                If UserList(TempCharIndex).flags.PertAlCons > 0 Then
                    Stat = Stat & " (Consejo Del Bien) -" & " El " & UserList(TempCharIndex).Clase & " " & UserList(TempCharIndex).Raza & FONTTYPE_CONSEJOVesA
                ElseIf UserList(TempCharIndex).flags.PertAlConsCaos > 0 Then
                    Stat = Stat & " (Consejo Del Mal) -" & " El " & UserList(TempCharIndex).Clase & " " & UserList(TempCharIndex).Raza & FONTTYPE_CONSEJOCAOSVesA
                Else
                   If UserList(TempCharIndex).flags.Privilegios > 0 Then
                   
                     If UserList(TempCharIndex).flags.Privilegios = 3 Then
                            Stat = Stat & " - (GM) - (Dios) - El " & UserList(TempCharIndex).Clase & " " & UserList(TempCharIndex).Raza & "~184~180~182~1~0"
                ElseIf UserList(TempCharIndex).flags.Privilegios = 2 Then
                            Stat = Stat & " - (GM) - (SemiDios) - El " & UserList(TempCharIndex).Clase & " " & UserList(TempCharIndex).Raza & "~105~227~74~1~0"
                ElseIf UserList(TempCharIndex).flags.Privilegios = 1 Then
                            Stat = Stat & " - (GM) - (Consejero) - El " & UserList(TempCharIndex).Clase & " " & UserList(TempCharIndex).Raza & "~235~233~234~1~0"
         End If

ElseIf Criminal(TempCharIndex) Then

Stat = Stat & " - <CRIMINAL> " & "-" & " El " & UserList(TempCharIndex).Clase & " " & UserList(TempCharIndex).Raza & "~255~0~0~1~0"
Else
Stat = Stat & " - <CIUDADANO> " & "-" & " El " & UserList(TempCharIndex).Clase & " " & UserList(TempCharIndex).Raza & "~0~0~200~1~0"
End If
            End If
